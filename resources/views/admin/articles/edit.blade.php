...
@section('content')
    <h1 class="font-heading text-xl text-brand-900 mb-6">{{ __('Edit') }}: {{ $article->title_en }}</h1>
    ...
    <form method="POST" action="{{ route('admin.articles.update', $article) }}" enctype="multipart/form-data" class="bg-white rounded-sm shadow-sm p-6 space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select id="issue_id" name="issue_id" required
                ...
                @foreach($issues as $issue)
                    <option value="{{ $issue->id }}" {{ old('issue_id', $article->issue_id) == $issue->id ? 'selected' : '' }}>{{ $issue->label }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input id="title_ka" type="text" name="title_ka" value="{{ old('title_ka', $article->title_ka) }}" required
                       class="block w-full rounded-sm border-brand-900/20 focus:border-brand-500 focus:ring-brand-500">
            </div>
            <div>
                <label for="title_en" class="block text-sm text-brand-900/70 mb-1">{{ __('English title') }}</label>
                <input id="title_en" type="text" name="title_en" value="{{ old('title_en', $article->title_en) }}" required
                       class="block w-full rounded-sm border-brand-900/20 focus:border-brand-500 focus:ring-brand-500">
            </div>
        </div>
        ...
        <div>
            ...
            <input id="authors" type="text" name="authors" value="{{ old('authors', $article->authors) }}" required
                   class="block w-full rounded-sm border-brand-900/20 focus:border-brand-500 focus:ring-brand-500">
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <textarea id="abstract_ka" name="abstract_ka" rows="4"
                          class="block w-full rounded-sm border-brand-900/20 focus:border-brand-500 focus:ring-brand-500">{{ old('abstract_ka', $article->abstract_ka) }}</textarea>
            </div>
            <div>
                ...
                <textarea id="abstract_en" name="abstract_en" rows="4"
                          class="block w-full rounded-sm border-brand-900/20 focus:border-brand-500 focus:ring-brand-500">{{ old('abstract_en', $article->abstract_en) }}</textarea>
            </div>
        </div>
        ...
        <div>
            ...
            <input id="sort_order" type="number" name="sort_order" value="{{ old('sort_order', $article->sort_order) }}"
                   class="block w-full rounded-sm border-brand-900/20 focus:border-brand-500 focus:ring-brand-500">
        </div>
        ...
        <div>
            ...
            @if($article->pdf_path)
                <p class="text-sm text-brand-900/60 mb-1">
                    <a href="{{ asset('storage/'.$article->pdf_path) }}" target="_blank" class="text-brand-600 hover:underline">{{ __('Download PDF') }}</a>
                </p>
            @endif
        ...
        <div>
            ...
            @if($article->cover_image_path)
                <img src="{{ asset('storage/'.$article->cover_image_path) }}" alt="" class="h-24 w-auto rounded-sm mb-2 object-cover">
            @endif
