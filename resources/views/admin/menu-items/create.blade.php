...
@section('content')
    <h1 class="font-heading text-xl text-brand-900 mb-6">{{ __('Create') }}: {{ __('Menu') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.menu-items.store') }}" class="bg-white rounded-sm shadow-sm p-6 space-y-4"
          x-data="{ linkType: '{{ old('link_type', 'internal') }}' }">
        @csrf
        ...
        <div x-show="linkType === 'internal'">
            ...
            <select id="internal_target" name="internal_target"
                ...
                @foreach($targets as $key => $target)
                    <option value="{{ $key }}" {{ old('internal_target') === $key ? 'selected' : '' }}>{{ $target['label_en'] }} / {{ $target['label_ka'] }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div x-show="linkType === 'custom'">
            ...
            <input id="custom_url" type="text" name="custom_url" value="{{ old('custom_url') }}" placeholder="https://..."
                   class="block w-full rounded-sm border-brand-900/20 focus:border-brand-500 focus:ring-brand-500">
        </div>
        ...
        <div>
            ...
            <select id="parent_id" name="parent_id"
                ...
                @foreach($parents as $parent)
                    <option value="{{ $parent->id }}" {{ (string) old('parent_id') === (string) $parent->id ? 'selected' : '' }}>{{ $parent->label_en }} / {{ $parent->label_ka }}</option>
                @endforeach
            </select>
        </div>
