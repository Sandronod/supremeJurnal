    ...
    <form method="POST" action="{{ route('admin.issues.update', $issue) }}" enctype="multipart/form-data" class="bg-white rounded-sm shadow-sm p-6 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input id="title_ka" type="text" name="title_ka" value="{{ old('title_ka', $issue->title_ka) }}"
                       class="block w-full rounded-sm border-brand-900/20 focus:border-brand-500 focus:ring-brand-500">
            </div>
            <div>
                <label for="title_en" class="block text-sm text-brand-900/70 mb-1">{{ __('English title') }}</label>
                <input id="title_en" type="text" name="title_en" value="{{ old('title_en', $issue->title_en) }}"
                       class="block w-full rounded-sm border-brand-900/20 focus:border-brand-500 focus:ring-brand-500">
            </div>
        </div>
